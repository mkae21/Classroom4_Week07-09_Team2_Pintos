...
1:1 - swap
*/
/* 채널 번호가 CHAN_NO인 디스크에 DEV_NO라는 번호가 붙은 디스크를
   반환합니다. (마스터 또는 슬레이브의 경우 각각 0 또는 1)
...
/* Returns the size of disk D, measured in DISK_SECTOR_SIZE-byte
   sectors. */
/* DISK_SECTOR_SIZE 바이트 섹터로 측정된 디스크 D의 크기를 반환합니다. */
...
/* 디스크 D에서 섹터 SEC_NO를 BUFFER로 읽습니다. 여기에는
   DISK_SECTOR_SIZE 바이트를 위한 공간이 있어야 합니다.
   내부적으로 디스크에 대한 액세스를 동기화하므로 외부의 디스크별
   잠금이 필요하지 않습니다. */
...
/* Selects device D, waiting for it to become ready, and then
   ...
   use LBA mode.) */
/* 장치 D를 선택하고 준비가 될 때까지 기다린 다음 디스크의
   섹터 선택 레지스터에 SEC_NO를 씁니다. (LBA 모드를 사용합니다.) */
...
/* PIO 모드에서 채널 C의 데이터 레지스터에서 섹터를 SECTOR로 읽으며,
   여기에는 DISK_SECTOR_SIZE 바이트를 위한 공간이 있어야 합니다. */
...
/* Wait up to 10 seconds for the controller to become idle, that
   ...
   pending interrupt. */
/* 컨트롤러가 유휴 상태가 될 때까지, 즉 상태 레지스터에서 BSY 및
   DRQ 비트가 지워질 때까지 최대 10초간 기다립니다.

   부작용으로 상태 레지스터를 읽으면 보류 중인 인터럽트가
   모두 지워집니다. */
...
/* Wait up to 30 seconds for disk D to clear BSY,
   ...
   complete its reset. */
/* 디스크 D가 BSY를 지울 때까지 최대 30초간 기다린 다음
   DRQ 비트의 상태를 반환합니다. ATA 표준에 따르면 디스크가
   재설정을 완료하는 데 그 정도 시간이 걸릴 수 있습니다. */
...
/* 이제 D가 선택된 디스크가 되도록 D의 채널을 프로그래밍합니다. */
...
/* select_device()로 해당 채널에서 디스크 D를 선택하되, 채널이
   유휴 상태가 될 때까지 기다립니다. */
