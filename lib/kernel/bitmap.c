...
/* B의 비트 번호가 지정된 IDX를 VALUE로 원자적으로 설정합니다. */
...
/* B의 비트 번호 BIT_IDX를 참으로 원자적으로 설정합니다. */
void bitmap_mark(struct bitmap *b, size_t bit_idx)
{
	...
	/* 이것은 단일 프로세서 시스템에서 원자성이 보장된다는 점을
	   제외하면 `b->bits[idx] |= mask'와 동일합니다.
	   OR 명령어에 대한 설명은 [IA32-v2b]를 참조하십시오. */
	asm("lock orq %1, %0" : "=m"(b->bits[idx]) : "r"(mask) : "cc");
}

/* Atomically sets the bit numbered BIT_IDX in B to false. */
/* B에서 비트 번호가 지정된 BIT_IDX를 거짓으로 원자적으로 설정합니다. */
void bitmap_reset(struct bitmap *b, size_t bit_idx)
{
	...
	/* 이것은 유니프로세서 머신에서 원자성이 보장된다는 점을 제외하면
	   `b->bits[idx] &= ~mask'와 동일합니다. AND 명령어에 대한 설명은
	   [IA32-v2a]를 참고하세요. */
...
/* START와 START + CNT 사이의 B 비트(배타적)가 VALUE로 설정된 경우
...
/* START와 START + CNT 사이의 B의 모든 비트(배타적)가 참으로
   설정되면 참을 반환하고, 그렇지 않으면 거짓을 반환합니다. */
...
/* 모두 VALUE로 설정된 START 또는 그 이후의 B에서 CNT 연속 비트의
   첫 번째 그룹의 시작 인덱스를 찾아서 반환합니다. 그러한 그룹이
   없으면 BITMAP_ERROR를 반환합니다. */
...
/* Finds the first group of CNT consecutive bits in B at or after
   ...
   setting them. */
/* START 이후 B에서 모두 VALUE로 설정된 CNT 연속 비트의 첫 번째
   그룹을 찾아서 모두 !VALUE로 뒤집은 다음 그룹 내 첫 번째 비트의
   인덱스를 반환합니다. 그러한 그룹이 없으면 BITMAP_ERROR를
   반환합니다. CNT가 0이면 0을 반환합니다. 비트는 원자적으로
   설정되지만 비트 테스트는 비트 설정과 함께 원자적으로 수행되지
   않습니다. */
