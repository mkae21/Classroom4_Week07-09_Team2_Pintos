...
/* Breaks a string into tokens separated by DELIMITERS.  The
   ...
   문자열을 DELIMITERS로 구분된 토큰으로 분리합니다. 이 함수를 처음 호출할 때 
   S는 토큰화할 문자열이어야 하며, 이후의 호출에서는 반드시 널 포인터이어야 합니다.

   SAVE_PTR은 토크나이저의 위치를 추적하는 데 사용되는 `char *` 변수의 주소입니다. 
   각 호출에서 반환되는 값은 문자열에서의 다음 토큰이며, 남아있는 토큰이 없으면 널 포인터입니다.

   이 함수는 여러 연속적인 구분자를 단일 구분자로 취급합니다.
   반환된 토큰은 절대로 길이 0일 수 없습니다. DELIMITERS는 문자열 내에서 한 호출에서 
   다음으로 변경될 수 있습니다.

   strtok_r()은 문자열 S를 수정하여 구분자를 널 바이트로 변경합니다. 
   따라서 S는 수정 가능한 문자열이어야 합니다. 특히 C에서 문자열 리터럴은 수정 가능하지 않지만,
   하위 호환성을 위해 `const`가 아닙니다.
