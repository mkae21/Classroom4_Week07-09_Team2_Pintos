 ...
 * allocation fails. */
/* 커널 가상 주소에 대한 매핑은 있지만 사용자 가상 주소에 대한 매핑은
 * 없는 새 페이지 맵 레벨 4(pml4)를 생성합니다. 새 페이지 디렉터리를
 * 반환하거나 메모리 할당이 실패하면 널 포인터를 반환합니다.*/
...
/* pml4e를 파괴하여 참조하는 모든 페이지를 해제합니다. */
void pml4_destroy(uint64_t *pml4)
{
	...
	/* PML4 (vaddr) >= 1이면, 정의에 의한 커널 공간입니다. */
 ...
 * failed. */
/* 사용자 가상 페이지 UPAGE에서 커널 가상 주소 KPAGE로 식별되는
 * 물리적 프레임에 페이지 맵 레벨 4 PML4의 매핑을 추가합니다.
 * UPAGE는 이미 매핑되어 있지 않아야 합니다.
 * KPAGE는 palloc_get_page()로 사용자 풀에서 가져온 페이지여야 합니다.
 * WRITABLE이 참이면 새 페이지는 읽기/쓰기 가능하지만,
 * 그렇지 않으면 읽기 전용입니다.
 * 성공하면 참을, 메모리 할당에 실패하면 거짓을 반환합니다. */
