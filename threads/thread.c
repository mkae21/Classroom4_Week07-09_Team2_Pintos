...
/* 실행 중인 스레드를 반환합니다.
   CPU의 스택 포인터 `rsp'를 읽은 다음 이를 반올림하여 페이지의
   시작 부분으로 내립니다. `struct thread`는 항상 페이지의 시작
   부분에 있고 스택 포인터는 중간 어딘가에 있기 때문에, 이것은
   현재 스레드를 찾습니다. */
...
/* 유휴 스레드. 실행할 준비가 된 다른 스레드가 없을 때 실행됩니다.
   ...
   처음에 한 번 스케줄링되며, 이 시점에서 idle_thread를 초기화하고
   전달된 세마포어를 "ups"하여 thread_start()가 계속할 수 있도록 한
   다음 즉시 차단합니다. 그 이후에는 유휴 스레드가 준비 목록에
   나타나지 않습니다. 준비 목록이 비어 있을 때 특수한 경우로
...
/* Chooses and returns the next thread to be scheduled.  Should
   ...
   idle_thread. */
/* 예약할 다음 스레드를 선택하고 반환합니다. 실행 대기열이 비어
   있지 않으면 실행 대기열에서 스레드를 반환해야 합니다. (실행
   중인 스레드가 계속 실행될 수 있으면 실행 대기열에 남아 있게
   됩니다.) 실행 대기열이 비어 있으면 idle_thread를 반환합니다. */
static struct thread *next_thread_to_run(void)
{
...
/* 새 스레드의 페이지 테이블을 활성화하여 스레드를 전환하고,
   이전 스레드가 dying 상태라면 스레드를 소멸시킵니다.

   이 함수를 호출할 때 방금 이전 스레드에서 새 스레드가 실행
   중이고 인터럽트는 여전히 비활성화되어 있습니다.

   따라서 스레드 전환이 완료될 때까지 printf()를 호출하는 것은
   안전하지 않습니다. 실제로는 함수 끝에 printf()를 추가해야
   한다는 뜻입니다. */
static void
...
/* 새 프로세스를 예약합니다. 진입 시 인터럽트가 꺼져 있어야 합니다.
   ...
   schedule() 함수에서 printf()를 호출하는 것은 안전하지 않습니다. */
static void do_schedule(int status)
{
	ASSERT(intr_get_level() == INTR_OFF);
